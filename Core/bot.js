...
module.exports = class ReiNaRework {
    constructor(option) {
        ...
        this.server = http.createServer((req, res) => {
            ...
            if (path.startsWith("/music/")) {
                ...
                if (!serverQueue) {
                    ...
                    fs.readFile(
                        `${__dirname}/../WebSocket/Music.html`,
                        (err, data) => {
                            if (err) {
                                res.writeHead(404);
            ...
            if (path.startsWith("/track/")) {
                ...
                if (
                    queryObj &&
                    queryObj.GID &&
                    this.bot.guilds.cache.get(queryObj.GID) &&
                    queryObj.TC &&
                    this.bot.channels.cache.get(queryObj.TC) &&
                    queryObj.VC &&
                    this.bot.channels.cache.get(queryObj.VC)
                ) {
                    play(this, queryObj);
                } else {
                    return;
                ...
                async function play(ReiNa, queryObj) {
                    ReiNa.util.handleVideo(
                        await ReiNa.util.getVideo(queryObj.id),
                        {
                            guild: ReiNa.bot.guilds.cache.get(queryObj.GID),
                            channel: ReiNa.bot.channels.cache.get(queryObj.TC),
                        },
                        ReiNa.bot.user,
                        ReiNa.bot.channels.cache.get(queryObj.VC),
                        false,
                        0
                    );
                }
            ...
            if (path === "/") {
                fs.readFile(
                    `${__dirname}/../WebSocket/Discord.html`,
                    (err, data) => {
                        if (err) {
                            res.writeHead(404);
                            res.write("404 Server Error");
        ...
        // this.bot = new Discord.Client({ intents: [Discord.Intents.FLAGS.DIRECT_MESSAGES, Discord.Intents.FLAGS.GUILDS, Discord.Intents.FLAGS.GUILD_MEMBERS, Discord.Intents.FLAGS.GUILD_VOICE_STATES] });
        this.bot = new Discord.Client({ intents: ["GUILDS", "GUILD_MESSAGES", "DIRECT_MESSAGES", 'GUILD_MESSAGE_REACTIONS', 'GUILD_EMOJIS_AND_STICKERS', 'GUILD_VOICE_STATES'], partials: ["CHANNEL"] });
        ...
        this.util.load().then((data) => {
            ...
            this.bot.on("messageCreate", (message) => {
                            ...
                            for (i = 0; i < message.attachments.size; i++) {
                                if (message.content) {
                                    this.bot.users.cache
                                        .get(this.config.ownerID)
                                        .send(
                                            `**${message.author.tag}**:\n${
                                                message.attachments.array()[i]
                                                    .url
                                            }\n${message.content}`
                                        );
                                } else {
                ...
                //     console.log(
                //         `${this.util.color.FgYellow}${this.util.getTime()}${
                //             this.util.color.Reset
                //         } ${this.util.color.FgCyan}${nHentai.name}${
                //             this.util.color.Reset
                //         } 指令被 ${message.author.tag}(${
                //             message.author.id
                //         }) 觸發!`
                //     );
                // } else {
                ...
                //             `${this.util.color.FgYellow}${this.util.getTime()}${
                //                 this.util.color.Reset
                //             } ${this.util.color.FgCyan}${wnacg.name}${
                //                 this.util.color.Reset
                //             } 指令被 ${message.author.tag}(${
                //                 message.author.id
                //             }) 觸發!`
                ...
                if (message.guild.id === "407171840746848258")
                    MSGsync.run(this, message);

                const CheckUserHuman = require("../Customize/MessageChecking/CheckUserHuman.js");
                if (
                    message.channel.id === "702962295998906398" &&
                    message.content === `${this.config.prefix}驗證`
                ) {
                    CheckUserHuman.run(this, message);
                    console.log(
                        `${this.util.color.FgYellow}${this.util.getTime()}${
                            this.util.color.Reset
                        } ${this.util.color.FgCyan}${CheckUserHuman.name}${
                            this.util.color.Reset
                        } 指令被 ${message.author.tag}(${
                            message.author.id
                        }) 觸發!`
                ...
                if (
                    message.content.includes("pixiv.net") ||
                    (message.attachments.size > 0 &&
                        message.content.includes("來源"))
                ) {
                    Sauce.PicFind(this, message);
                    console.log(
                        `${this.util.color.FgYellow}${this.util.getTime()}${
                            this.util.color.Reset
                        } ${this.util.color.FgCyan}${Sauce.name}${
                            this.util.color.Reset
                        } 指令被 ${message.author.tag}(${
                            message.author.id
                        }) 觸發!`
                ...
                if (!message.author.bot)
                    ChatLog.log(
                        `./ChatLog/${message.guild.id}/${message.channel.id}.log`,
                        message
                    );
                ...
                if (message.guild.id === "398062441516236800") {
                    if (
                        ...
                        message.content
                            .toLowerCase()
                            .includes("discordapp.com/invite/") ||
                        message.content
                            .toLowerCase()
                            .includes("discord.com/invite/")
                    ) {
                        message.delete().catch();
                        let NotAllow = this.util.createEmbed(
                            message.author,
                            null,
                            `這裡不允許發送Discord邀請連結!`,
                            null,
                ...
                if (command) {
                    ...
                    console.log(
                        `${this.util.color.FgYellow}${this.util.getTime()}${
                            this.util.color.Reset
                        } ${this.util.color.FgCyan}${command.name}${
                            this.util.color.Reset
                        } 指令被 ${message.author.tag}(${
                            message.author.id
                        }) 觸發!`
            ...
            this.bot.on("guildCreate", (guild) => {
                console.log(
                    `${this.util.color.FgYellow}${this.util.getTime()}${
                        this.util.color.Reset
                    } ${this.util.color.FgCyan}加入伺服器 |${guild.name}(${
                        guild.id
                    })|()`
                );
            });
            this.bot.on("guildDelete", (guild) => {
                console.log(
                    `${this.util.color.FgYellow}${this.util.getTime()}${
                        this.util.color.Reset
                    } ${this.util.color.FgCyan}離開伺服器 |${guild.name}(${
                        guild.id
                    })|()`
                );
            });
            this.bot.on("voiceStateUpdate", (oldState, newState) => {
                ...
                if (
                    oldState.member.user != this.bot.user ||
                    newState.member.user != this.bot.user
                )
                    return;
                if (!newState.channelID) {
                    let disconnect = this.util.createEmbed(
                        null,
                        null,
                        `Senpai, 我被管理員中斷語音連接了! 如果下次需要我的時候再叫我吧！\n\n\n**此信息將會在5秒後自動刪除**\n`,
                        null,
                        0xcc0000
                    );
                    this.queue
                        .get(oldState.guild.id)
                        .textChannel.send(disconnect)
                        .then((msg) => {
                ...
                if (oldState.channelID != newState.channelID) {
                    try {
                        this.queue.get(newState.guild.id).voiceChannel =
                            newState.channel;
                    } catch (e) {}
                }
            });
        ...
        this.event.on("WWarning", async (warningMSG) => {
            ...
            let Warning = this.util.createEmbed(
                null,
                `ReiNa Bot Rework 天氣警告`,
                `自動警告 ${new Date()}`
            );
