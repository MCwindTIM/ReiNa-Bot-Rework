    ...
    async run(message, args, prefix){
        ...
        if(url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/) || url.match(/^https:\/\/(?:www\.)?youtube\.com\/watch\?((v=[^&\s]*&list=[^&\s]*)|(list=[^&\s]*&v=[^&\s]*))(&[^&\s]*)*$/)){
            ...
            if(!serverQueue){
                ...
                await serverQueue.songs.splice(1, 0, Newestsong);
                serverQueue = await this.main.util.getServerQueue(message.guild.id);
				setTimeout(async ()=> {
                    //renew serverQueue
					let done = await this.main.util.createEmbed(message.author, null, `${message.author} Senpai, 已經為你設置\n<@${serverQueue.songs[1].author.id}>添加的**${serverQueue.songs[1].title}** 為下一個播放的音樂!`, null, 0xcc0000);
